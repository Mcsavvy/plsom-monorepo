...
import { LogOut, Settings, User, BookOpen, Calendar, Clock, Play, AlertCircle, ExternalLink, Timer } from "lucide-react";
          ...
          <div className="flex items-center space-x-4">
            <Avatar className="size-26">
              {user?.profilePicture ? (
                ...
                <div className="flex items-center justify-center h-full w-full bg-primary text-primary-foreground font-semibold text-4xl">
                  {user?.initials}
                </div>
              )}
            </Avatar>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    ...
                    <div className="absolute -top-1 -left-1 h-5 w-5 bg-green-500/30 rounded-full animate-ping"></div>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-green-600 dark:text-green-300">
                      {liveClass.title} with {liveClass.instructor} • Started at {liveClass.startTime}
                    </p>
                  </div>
                </div>
                <Button
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                ...
                <div className="text-4xl font-bold text-primary">{coursesInProgress}</div>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                <h3 className="text-lg font-semibold mb-2">Upcoming Tests</h3>
                <div className="text-4xl font-bold text-primary">{upcomingTests}</div>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {ongoingTests.map((test) => (
                <Card key={test.id} className={`${test.isSubmitted
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                        ...
                        {!test.isSubmitted && (
                          <div className="flex items-center space-x-2 text-orange-600 dark:text-orange-400">
                            ...
                            <span className="font-medium text-sm">{test.timeRemaining} remaining</span>
                          </div>
                        )}
                        {test.isSubmitted && (
                          <div className="flex items-center space-x-2 text-green-600 dark:text-green-400">
                            <AlertCircle className="h-4 w-4" />
                            <span className="font-medium text-sm">{test.timeRemaining}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div className="flex items-center space-x-4 text-sm">
                          ...
                          <span className={`font-medium ${test.isSubmitted ? 'text-green-600' : 'text-orange-600'}`}>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-orange-600 dark:text-orange-300">
                    Finish setting up your profile to access all features of the platform.
                  </p>
                </div>
              </div>
            </CardContent>
