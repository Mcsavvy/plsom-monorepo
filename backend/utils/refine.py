    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        if start is not None and end is not None:
            try:
                ...
                if paginator.count > 0 and self.page.start_index() > paginator.count:
                    raise NotFound(self.invalid_page_message.format(
...
class RefineDataProviderFilter(filters.BaseFilterBackend):
    ...
    def apply_filtering(self, request, queryset, view):
        ...
        for param, value in request.query_params.items():
            if param in ['_start', '_end', '_sort', '_order', 'q', 'page', 'page_size']:
                continue
            ...
            if method.upper() == 'GET':
                ...
                if hasattr(self.target, 'queryset') and self.target.queryset is not None:
                    ...
                    for field in model._meta.fields:
                        ...
                        if field.get_internal_type() in ['IntegerField', 'FloatField', 'DecimalField', 'DateField', 'DateTimeField']:
                            parameters.extend([
                                OpenApiParameter(
