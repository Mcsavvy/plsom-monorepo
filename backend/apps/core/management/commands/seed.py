...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            choices=[
                'users', 'cohorts', 'enrollments', 'courses', 
                'classes', 'invitations', 'tests', 'questions', 
                'question_options'
            ]
        )
    ...
    def handle(self, *args, **options):
        if not settings.DEBUG:
            self.stdout.write(
                self.style.ERROR('This command can only be run in DEBUG mode (development)')
            )
            return
    ...
    def seed_from_file(self, file_path, filename):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully seeded {len(data)} items from {filename}.json')
            )
    ...
    def create_object(self, item, filename):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'Error creating {model_name} with pk {pk}: {str(e)}')
            )
            raise
    ...
    def parse_datetime_fields(self, fields, model_name):
        ...
        datetime_fields = {
            ...
            'assessments.test': ['available_from', 'available_until', 'created_at', 'updated_at'],
        }
                ...
                try:
                    ...
                    if field_name in ['start_date', 'end_date']:
                        if isinstance(value, str):
                            ...
                            fields[field_name] = datetime.strptime(value, '%Y-%m-%d').date()
                        ...
                        if isinstance(value, str):
                            ...
                            if value.endswith('Z'):
                                ...
                                value = value[:-1] + '+00:00'
                            fields[field_name] = datetime.fromisoformat(value.replace('Z', '+00:00'))
                        elif isinstance(value, datetime):
                            fields[field_name] = value
                        elif isinstance(value, date):
                            # Convert date to datetime at midnight
                            fields[field_name] = datetime.combine(value, datetime.min.time())
                            
                except (ValueError, TypeError) as e:
                    self.stdout.write(
                        self.style.WARNING(f'Could not parse datetime field {field_name} with value {fields[field_name]}: {str(e)}')
                    )
    ...
    def create_question(self, pk, fields):
        ...
        question, created = Question.objects.get_or_create(pk=pk, defaults=fields)
        return question
