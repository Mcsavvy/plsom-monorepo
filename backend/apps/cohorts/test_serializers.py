...
class CohortSerializerTestCase(TestCase):
    ...
    def test_validate_name_new_cohort_duplicate(self):
        ...
        self.assertIn(
            "already exists for certificate program", str(serializer.errors)
        )
    ...
    def test_serializer_update(self):
        ...
        self.assertEqual(
            updated_cohort.end_date, self.far_future_date + timedelta(days=10)
        )
    ...
    def test_serializer_partial_update(self):
        ...
        self.assertEqual(
            updated_cohort.program_type, "certificate"
        )  # Unchanged
        self.assertEqual(
            updated_cohort.start_date, self.future_date
        )  # Unchanged
    ...
    def test_enrollment_validation(self):
        ...
        if serializer.is_valid():
            try:
                serializer.save()
                self.fail(
                    "Should have raised IntegrityError for duplicate enrollment"
                )
            except Exception:
                # Expected to fail
                pass
