...
class CohortViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.enrollment = Enrollment.objects.create(
            student=self.student, cohort=self.cohort1
        )
    ...
    def test_create_cohort_overlapping_dates(self):
        ...
        data = {
            ...
            + timedelta(days=40),  # Overlaps with cohort1
        }
    ...
    def test_partial_update_cohort_as_admin(self):
        ...
        self.assertEqual(
            response.data["program_type"], "certificate"
        )  # Unchanged
    ...
    def test_archive_cohort_as_admin_success(self):
        ...
        response = self.client.post(
            f"/api/cohorts/{self.active_cohort.id}/archive/"
        )
...
class EnrollmentViewSetTestCase(APITestCase):
    ...
    def test_enrollment_viewset_is_readonly(self):
        ...
        data = {"student": self.student1.id, "cohort": self.cohort.id}
        ...
        response = self.client.patch(
            f"/api/enrollments/{self.enrollment1.id}/", data
        )
        self.assertEqual(
            response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED
        )
        ...
        response = self.client.delete(
            f"/api/enrollments/{self.enrollment1.id}/"
        )
        self.assertEqual(
            response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED
        )
