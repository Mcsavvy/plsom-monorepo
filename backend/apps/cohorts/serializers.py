...
class CohortSerializer(serializers.ModelSerializer):
    ...
    def validate_start_date(self, value):
        ...
        if instance and instance.start_date <= today and value != instance.start_date:
            raise serializers.ValidationError("Cannot change start date for a cohort that has already started.")
    ...
    def validate(self, attrs):
        ...
        if instance:
            ...
            is_active = is_active if is_active is not None else instance.is_active
    ...
    def validate(self, attrs):
        ...
        if student and cohort:
            ...
            if not cohort.is_active:
                raise serializers.ValidationError("Cannot enroll in an inactive cohort.")
            
            # Cannot enroll in ended cohort
            today = timezone.now().date()
            if cohort.end_date and cohort.end_date < today:
                raise serializers.ValidationError("Cannot enroll in a cohort that has already ended.")
