...
class ClassJoinTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.class_obj = Class.objects.create(
            ...
            + timedelta(minutes=10),  # Class starts in 10 minutes
            duration_minutes=90,
    ...
    def test_student_join_class_creates_attendance(self):
        ...
        attendance = Attendance.objects.get(
            class_session=self.class_obj, student=self.student
        )
        self.assertIsNotNone(attendance)
        self.assertEqual(attendance.join_time.date(), self.now.date())
        # Leave time should be set to class end time
        expected_leave_time = self.class_obj.scheduled_at + timedelta(
            minutes=self.class_obj.duration_minutes
        )
        ...
        self.assertLessEqual(
            attendance.duration_minutes, self.class_obj.duration_minutes
        )
    ...
    def test_student_can_join_class_with_recording(self):
        ...
        self.assertTrue(response.data["attendance_registered"])
        self.assertEqual(
            response.data["recording_url"], "https://example.com/recording"
        )

        # Check that attendance record was created with via_recording=True
        attendance = Attendance.objects.get(
            class_session=past_class, student=self.student
        )
        self.assertTrue(attendance.via_recording)
        # Leave time should be set to class end time even for recordings
        expected_leave_time = past_class.scheduled_at + timedelta(
            minutes=past_class.duration_minutes
        )
        self.assertEqual(attendance.leave_time, expected_leave_time)
        # Duration should be full class duration for recordings
        self.assertEqual(
            attendance.duration_minutes, past_class.duration_minutes
        )
...
class ClassViewSetTestCase(APITestCase):
    ...
    def test_update_recording_link_after_class_completion(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["recording_url"], "https://example.com/recording.mp4"
        )
        self.assertEqual(
            response.data["password_for_recording"], "recording123"
        )

        # Verify the update was saved
        past_class.refresh_from_db()
        self.assertEqual(
            past_class.recording_url, "https://example.com/recording.mp4"
        )
    ...
    def test_update_restricted_fields_after_class_completion(self):
        ...
        data = {
            "scheduled_at": self.now
            + timedelta(hours=1),  # Try to reschedule (should be ignored)
        ...
        self.assertEqual(
            past_class.duration_minutes, 90
        )  # Should remain unchanged
        self.assertEqual(
            past_class.scheduled_at, self.now - timedelta(hours=2)
        )  # Should remain unchanged
    ...
    def test_update_allowed_fields_after_class_completion(self):
        ...
        self.assertEqual(
            response.data["description"], "Updated description for past class"
        )
        self.assertEqual(
            response.data["recording_url"], "https://example.com/recording.mp4"
        )
...
class AttendanceViewSetTestCase(APITestCase):
    ...
    def test_verify_attendance_as_lecturer(self):
        ...
        response = self.client.patch(
            f"/api/attendance/{self.attendance.id}/", data
        )
        print(response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_verify_attendance_as_student_forbidden(self):
        ...
        data = {"verified": True, "notes": "Unauthorized verification"}
    ...
    def test_unverify_attendance_as_lecturer(self):
        ...
        data = {"verified": False, "notes": "Attendance needs review"}
    ...
    def test_class_attendance_summary_as_lecturer(self):
        ...
        response = self.client.get(
            f"/api/attendance/class-summary/?class_id={self.class_obj.id}"
        )
    ...
    def test_attendance_filtering_by_class(self):
        ...
        response = self.client.get(
            f"/api/attendance/?class_session={self.class_obj.id}"
        )
    ...
    def test_attendance_filtering_by_student(self):
        ...
        response = self.client.get(
            f"/api/attendance/?student={self.student.id}"
        )
...
class ClassJoinIntegrationTestCase(APITestCase):
    ...
    def test_join_class_workflow(self):
        ...
        self.assertTrue(response.data["attendance_registered"])
        self.assertEqual(
            response.data["zoom_join_url"], "https://zoom.us/j/123456789"
        )
        self.assertEqual(response.data["password_for_zoom"], "testpass")

        # Check that attendance was created
        attendance = Attendance.objects.get(
            class_session=class_obj, student=self.student
        )
        self.assertIsNotNone(attendance)
        self.assertEqual(
            attendance.duration_minutes, 90
        )  # Full duration since class hasn't started
    ...
    def test_join_class_recording_workflow(self):
        ...
        self.assertEqual(
            response.data["password_for_recording"], "recordingpass"
        )
    ...
    def test_join_class_multiple_times(self):
        ...
        self.assertEqual(response2.status_code, status.HTTP_200_OK)
        self.assertFalse(
            response2.data["attendance_registered"]
        )  # Already registered
