...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not all([email, password]):
            self.stdout.write(self.style.WARNING('ADMIN_EMAIL and ADMIN_PASSWORD must be set in environment.'))
            return

        if User.objects.filter(email=email, is_superuser=True).exists():
            self.stdout.write(self.style.SUCCESS(f'Admin with email {email} already exists.'))
            return
