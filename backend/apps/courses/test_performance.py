...
class QueryCountTestCase(APITestCase):
    ...
    def test_my_courses_query_count_with_small_dataset(self):
        ...
        self.assertLess(
            query_count, 8, f"Too many queries for small dataset: {query_count}"
        )

        # Verify response structure
        self.assertLessEqual(
            len(response.data["results"]), 5
        )  # Allow for default pagination
    ...
    def test_my_courses_query_count_with_medium_dataset(self):
        ...
        self.assertLess(
            query_count,
            10,
            f"Too many queries for medium dataset: {query_count}",
        )
    ...
    def test_my_courses_query_count_with_large_dataset(self):
        ...
        self.assertLess(
            query_count,
            15,
            f"Too many queries for large dataset: {query_count}",
        )
    ...
    def test_my_courses_query_analysis(self):
        ...
        if (
            ...
            and response.data["results"]
        ):
            course = response.data["results"][0]
            has_optimized_fields = (
                "total_classes_in_my_cohorts" in course
                and "upcoming_classes_in_my_cohorts" in course
            )
            self.assertTrue(
                has_optimized_fields, "Optimized fields not found in response"
            )
        else:
            ...
            self.assertLess(
                len(queries),
                5,
                "Too many queries - optimization may not be working",
            )
    ...
    def test_my_courses_with_filters_query_count(self):
        ...
        self.assertLess(
            query_count,
            8,
            f"Too many queries with program_type filter: {query_count}",
        )
        ...
        self.assertLess(
            query_count,
            8,
            f"Too many queries with is_active filter: {query_count}",
        )
    ...
    def test_my_courses_pagination_query_count(self):
        ...
        query_count = len(connection.queries)
        self.assertLess(
            query_count, 10, f"Too many queries with pagination: {query_count}"
        )
        ...
        self.assertGreater(
            len(response.data["results"]), 0, "No results returned"
        )
        self.assertGreater(
            response.data["count"], 0, "Total count should be greater than 0"
        )
